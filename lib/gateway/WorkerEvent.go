...
func (w *WorkerEvent) OnStart(listen network.ListenTcp) {
	...
	if w.HandleFunc == nil {
		...
		w.HandleFunc[lib.CMD_SET_SESSION] = WorkerHandle.OnTodo
		w.HandleFunc[lib.CMD_UPDATE_SESSION] = WorkerHandle.OnTodo
		w.HandleFunc[lib.CMD_GET_SESSION_BY_CLIENT_ID] = WorkerHandle.OnTodo
		...
		w.HandleFunc[lib.CMD_SEND_TO_GROUP] = WorkerHandle.OnTodo
		w.HandleFunc[lib.CMD_GET_CLIENT_SESSIONS_BY_GROUP] = WorkerHandle.OnTodo
